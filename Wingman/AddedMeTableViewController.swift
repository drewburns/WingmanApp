...
class AddedMeTableViewController: UITableViewController {
    ...
    func getFriends() {
        print("here")
        let ref = base.child("added-friendships").child((user?.id)!)
        ref.observeSingleEvent(of: .value, with:{ (snapshot) in
            print(snapshot.value)
            if snapshot.exists() {
                print("made it to 1")
                if let friendStrings = snapshot.value as? [String:Any] {
                    print("made it to 2")
        ...
        print("WHERE ARE THE FRIENDS", users)
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "friend", for: indexPath) as! AddedMeTVC
    ...
    override func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if let cell = cell as? AddedMeTVC {
